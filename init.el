...
;;;; Config options
(require 'config-options)    ; Defines the config:* customization group
...
(require 'package-macros)    ; Defines convenience `use-package' macros
(require 'package-metadata)  ; Additional metadata
(require 'leader-key-system) ; Defines the macros used to create \"leader-key menus\"
...
(use-builtin package
  ...
  :general-config
  (nmap package-menu-mode
    "i" 'package-install
    "L" 'package-list-packages-no-fetch
    "l" 'package-list-packages
    "r" 'package-refresh-contents
    "R" 'use-package-report
    "p" 'describe-package)

  (nmap package-menu-mode
    :prefix "/"
    ""  '(:ignore t :which-key "Search")
    "d" '("Description" . package-menu-filter-by-description)
    "m" '("Marked"      . package-menu-filter-marked)
    "n" '("Name"        . package-menu-filter-by-name)
    "s" '("Status"      . package-menu-filter-by-status)
    "u" '("Upgrades"    . package-menu-filter-upgradable)))
...
(use-package consult
  ...
  ;; hide all except normal buffers in `consult-buffer' by default
...
;; [[file:org-org-roam.org::*Dailies][Dailies:1]]
(use-package org-roam-dailies
  ...
  (org-roam-dailies-capture-templates
   '(("d" "default" entry "* %?"
      :target
      (file "%<%Y-%m-%d>.org"
                 "%(org-id-get-create)#+TITLE: Journal: %<%b %d %Y (%a)>\n#+SUBTITLE: Daily Journal\n\n")
      :unnarrowed t)))
  :pretty-hydra
